...
/* ADDSUB :   SUB          */
#include<stdio.h>
void pg_fix_sub_20(int x,int y,int* z){
  unsigned int _x;
  unsigned int _y;
  int _z;
  _x = 0xFFFFF&x;
...
#include<stdio.h>                                                  
#include<math.h>                                                   
void pg_conv_ftol_fix20_log14_man5(int fixdata, int* logdata){                 
  int fixdata_msb=0;                                               
  int logdata_sign=0;                                              
  int fixdata_body=0;                                              
  int abs=0;                                                       
  ...
  /* SIGN BIT */                                                   
  fixdata_msb = 0x1&(fixdata >>19);         
  logdata_sign = fixdata_msb;                                      

  /* ABSOLUTE */                                                   
  fixdata_body    = 0x7FFFF & fixdata;        
  {                                                                
    int inv_fixdata_body=0;                                        
    inv_fixdata_body = 0x7FFFF ^ fixdata_body;
    if(fixdata_msb == 0x1){                                        
      abs = 0x7FFFF & (inv_fixdata_body + 1); 
    }else{                                                         
      abs = fixdata_body;                                          
    }                                                              
  }                                                                
  abs_decimal = 0x3FFFF& abs;                 
...
#include<stdio.h>                                
void pg_pdelay_14_np8(int indata, int* outdata){
...
#include<stdio.h>                                        
void pg_log_shift_log14_m1(int logimage_x, int* logimage_z){ 
  int x_sign=0;        /* Sign Flag:  1-bit (for debug)*/
  int x_nonzero=0;     /* NonZero Flag: 1-bit */         
  int x_body=0;        /* 12-bit */   
  int z=0;             /* 13-bit */   
...
#include<stdio.h>                                
void pg_pdelay_14_np12(int indata, int* outdata){
  *outdata = 0x3FFF&indata;      
  return;                                        
}          
/* Generated by PGPG module generateor       */
...
#include<stdio.h>                                                  
#include<math.h>                                                   
void pg_conv_ltof_log14_man5_fix49(int logdata, long long int* fixdata){       
  int logdata_sign=0;                                              
  int logdata_nonzero=0;                                           
  int logdata_exponent=0;                                          
  int logdata_mantissa=0;                                          
  ...
  {                                                                
    ...
    shift_out = seed >> 5;   
    fixdata_absolute = 0x0000FFFFFFFFFFFFULL & shift_out;   
  }                                                                

  *fixdata = (long long int)fixdata_sign << 48 | fixdata_absolute;
...
/* nbit_fx(input)   :   49-bit      */
/* nbit_sx(register):   56-bit      */
/* --- [NOTE!] ---
...
#include<stdio.h>                                                  
#include<math.h>                                                   
void pg_fix_accum_f49_s56(long long int fdata, long long int* sdata){ 
  long long int fx=0;     /* 49-bit */         
  long long int sx=0;     /* 56-bit */         
  long long int fx_sign=0;     /* 1-bit : sign flag of fdata */     
  long long int fx_abs=0;      /* 48-bit : absolute part of fdata */
  fx= 0x0001FFFFFFFFFFFFULL&fdata;                             
  sx= 0x00FFFFFFFFFFFFFFULL&(*sdata);                          
  fx_sign = 0x1&(fdata >>48);               
  fx_abs  = 0x0000FFFFFFFFFFFFULL&fdata;                    
  if(fx_sign == 0x0){                                              
    sx = sx + fx_abs;                                              
  }else{                                                           
    sx = sx - fx_abs;                                              
  ...
  *sdata = 0x00FFFFFFFFFFFFFFULL&sx;
  return;  
}          
